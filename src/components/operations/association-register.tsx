...
export function AssociationRegister({ canManage, projects, projectId, rows }: { canManage: boolean; projects: Array<{ id: string; name: string }>; projectId: string | null; rows: Row[] }) {
  ...
  return <RegisterScreen<Row> basePath="/association" projects={projects} projectId={projectId} canManage={canManage} rows={rows}
    ...
    tiles={[{ label: 'Charges', value: String(rows.length) }, { label: 'Overdue', value: String(overdue), tone: overdue > 0 ? 'bad' : 'good' }, { label: 'Billed', value: formatCurrency(rows.reduce((s, r) => s + r.amount, 0)) }]}
    columns={[{ label: 'Unit', render: (r) => r.unitCode }, { label: 'Period', render: (r) => r.period ?? '—' }, { label: 'Amount', render: (r) => formatCurrency(r.amount) }, { label: 'Due', render: (r) => fmt(r.dueOn) }, { label: 'Status', render: (r) => <StatusBadge status={r.status} /> }]}
