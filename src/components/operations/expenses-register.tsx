...
import { formatCompactCurrency as compactINR, formatCurrency } from '@/lib/utils/format';
...
export function ExpensesRegister({ canManage, projects, projectId, rows }: { canManage: boolean; projects: Array<{ id: string; name: string }>; projectId: string | null; rows: Row[] }) {
  ...
  return <RegisterScreen<Row> basePath="/expenses" projects={projects} projectId={projectId} canManage={canManage} rows={rows}
    ...
    columns={[{ label: 'Claimant', render: (r) => r.claimant }, { label: 'Category', render: (r) => r.category ?? '—' }, { label: 'Amount', render: (r) => formatCurrency(r.amount) }, { label: 'Incurred', render: (r) => fmt(r.incurredOn) }, { label: 'Status', render: (r) => <StatusBadge status={r.status} /> }]}
