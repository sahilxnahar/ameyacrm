  ...
  if (done) {
    return (
      <p className="mt-4 flex items-center gap-2 rounded-md bg-emerald-50 p-3 text-sm text-emerald-900 dark:bg-emerald-950/40 dark:text-emerald-300">
        <Check className="h-4 w-4" />Password changed. Taking you to sign in…
      </p>
    );
  }
    ...
    <form onSubmit={submit} className="mt-4 space-y-3">
      <label className="block">
        ...
        <span className={`mt-1 block text-xs ${tooShort ? 'text-destructive' : 'text-muted-foreground'}`}>
          At least 12 characters. A short phrase you will remember is fine.
        </span>
      </label>
      ...
      >
        {pending ? <Loader2 className="mx-auto h-4 w-4 animate-spin" /> : 'Set my new password'}
      </button>
    </form>
  );
}
