...
export async function buildLetterPdf(d: LetterData): Promise<Uint8Array> {
  ...
  const wrap = (s: string, size: number, maxW: number) => { const out: string[] = []; let ln = ''; for (const w of s.split(' ')) { const t = ln ? ln + ' ' + w : w; if (font.widthOfTextAtSize(t, size) > maxW) { out.push(ln); ln = w; } else ln = t; } if (ln) out.push(ln); return out; };
  ...
  text(title, M, y, 14, bold, CHARCOAL); y -= 28;
  ...
  if (d.kind === 'DEMAND' && d.amountDue != null) {
    y -= 2; page.drawRectangle({ x: M, y: y - 4, width: W - 2 * M, height: 22, color: CHARCOAL });
    text('TOTAL NOW DUE', M + 8, y + 2, 11, bold, rgb(1, 1, 1)); right(money(d.amountDue), W - M - 8, y + 2, 12, bold, BRASS); y -= 34;
  } else if (d.agreementValue != null) {
    y -= 2; text('Agreement value', M + 8, y, 10, bold); right(money(d.agreementValue), W - M - 8, y, 11, bold); y -= 24;
  }

  y -= 20; text('For ' + d.company.name, M, y, 10); y -= 34; text('Authorised Signatory', M, y, 10, font, MUTE);
  y -= 30;
