import 'server-only';
import { formatCurrencyForPdf, formatQuantity } from '@/lib/utils/format';
...
// `Rs. Rs.1,50,000`.
const money = (n: number) => formatCurrencyForPdf(n); // ASCII only (StandardFonts lacks the rupee glyph)
...
export async function buildInvoicePdf(d: InvoicePdfData): Promise<Uint8Array> {
  ...
  for (const it of d.items) {
    ...
    right(formatQuantity(it.quantity), cols.qty + 30, y, 10);
