...
export async function POST(req: NextRequest) {
  ...
  // 2) Else: a booking id in the notes → its earliest unpaid milestone of that amount.
  if (!milestone && notes.bookingId) {
    milestone = await prisma.paymentMilestone.findFirst({
  ...
  if (milestone && milestone.status !== 'PAID') {
    await prisma.paymentMilestone.update({ where: { id: milestone.id }, data: { status: 'PAID', paidAt: new Date() } });
    await writeAudit({ action: 'UPDATE', entityType: 'PaymentMilestone', entityId: milestone.id, summary: `Razorpay ${pay.id}: ₹${rupees} reconciled to "${milestone.label}"` });
    return NextResponse.json({ ok: true, reconciled: milestone.id });
  }
