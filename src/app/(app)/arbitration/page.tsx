...
export default async function ArbitrationPage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  await requirePermission('land.view');
  const win = listWindow(await searchParams, 200);
  const [rows, adrCaseTotal, projects, vendors, hearingSoon] = await Promise.all([
    prisma.adrCase.findMany({ orderBy: [{ nextHearingOn: 'asc' }], take: win.take, include: { project: { select: { name: true } }, vendor: { select: { name: true } } } }).catch(() => []),
    prisma.adrCase.count().catch(() => 0),
  ...
  return (
    <div className="space-y-6">
      ...
      <ArbitrationView projects={projects} vendors={vendors} counts={{ total: adrCaseTotal, hearingSoon }}
      ...
      <ListNotice meta={listMeta(rows.length, adrCaseTotal, win)} noun="matters" />
    </div>
  );
}
